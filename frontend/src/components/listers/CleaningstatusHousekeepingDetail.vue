<template>
    <v-card outlined>
        <v-card-title>
            Housekeeping # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Boolean label="Cleaned" v-model="item.cleaned" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/housekeepings/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
